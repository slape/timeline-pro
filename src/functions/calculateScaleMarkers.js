...
const calculateScaleMarkers = (startDate, endDate, scale) => {
  ...
  if (lastMarker.position < endPosition - 1) {
    // Small threshold to avoid duplicates
    scaleMarkers.push({
      date: new Date(end),
  ...
  if (scaleMarkers.length > 2) {
    ...
    for (let i = 1; i < scaleMarkers.length - 1; i++) {
      const distance =
        scaleMarkers[i].position - scaleMarkers[lastVisibleIndex].position;
