...
const fetchBoardItems = async (
  ...
  if (!dateColumnId) {
    TimelineLogger.warn(
      "fetchBoardItems: Could not resolve dateColumnId from input",
      { dateColumn },
    );
  }
  TimelineLogger.debug("Extracted date column ID", { dateColumnId });
  if (!context || !context.boardId) {
    TimelineLogger.warn("Invalid context provided to fetchBoardItems", {
      context,
    });
    return;
  }
      ...
      if (itemIds && itemIds.length > 0) {
        ...
        if (response.data && response.data.items) {
          ...
          TimelineLogger.debug("fetchBoardItems.keepingAllColumns", {
            ...
            availableColumns: response.data.items[0]?.column_values?.map(
              (cv) => cv.id,
            ),
          });
        } else {
          TimelineLogger.warn("No items found for the specified IDs", {
            itemIds,
          });
          setBoardItems([]);
        }
        ...
        if (
          ...
          TimelineLogger.debug("fetchBoardItems.keepingAllColumns", {
            dateColumnId,
            totalColumns:
              response.data.boards[0].items_page.items[0]?.column_values
                ?.length,
            availableColumns:
              response.data.boards[0].items_page.items[0]?.column_values?.map(
                (cv) => cv.id,
              ),
          });
        } else {
