  ...
  try {
    ...
    if (!dateColumnId) {
      TimelineLogger.error(
        "Cannot update item date: date column not available",
        {
          itemId: item.id,
    ...
    TimelineLogger.debug("Column type detection debug", {
      ...
      allColumnIds:
        item?.originalItem?.column_values?.map((col) => ({
          id: col.id,
          type: col.type,
        })) || [],
    });
    ...
    if (item?.originalItem?.column_values) {
      const columnValue = item.originalItem.column_values.find(
      ...
      TimelineLogger.debug("Found column value for date column", {
        ...
        columnValueParsed: columnValue?.value
    ...
    const result = await updateItemDate(
      monday,
      item.id,
      context.boardId,
      dateColumnId,
    ...
    if (result.success) {
      ...
      const newColumnValue = createColumnValue(
        dateToUpdate,
        columnType,
        currentColumnValue,
      );
      ...
      if (!newColumnValue) {
        TimelineLogger.error(
          "Failed to create column value for local store update",
          {
            dateToUpdate,
            columnType,
            currentColumnValue,
      ...
      TimelineLogger.debug(
        "Updated local board item data, timeline should re-render automatically",
        {
          itemId: item.id,
