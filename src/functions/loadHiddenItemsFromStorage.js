...
export default async function loadHiddenItemsFromStorage(
  storageService,
  HIDDEN_ITEMS_KEY,
) {
  if (!storageService) {
    TimelineLogger.debug(
      "Storage service not initialized, returning empty array",
    );
    return [];
  }
  ...
  try {
    ...
    if (response?.data?.success && response.data.value) {
      ...
      if (typeof hiddenItems === "string") {
        TimelineLogger.debug(
          "Monday storage returned string, converting to array",
          { rawValue: hiddenItems },
        );
      ...
      if (!Array.isArray(hiddenItems)) {
        TimelineLogger.warn(
          "Hidden items from storage is not an array, converting",
          { type: typeof hiddenItems, value: hiddenItems },
        );
        hiddenItems = [];
      }
    ...
    TimelineLogger.error(
      "Failed to load hidden items from Monday storage",
      error,
    );
    return [];
  }
