...
export default function buildItemToMarkerMap({
  ...
  if (visibleBoardItems && visibleBoardItems.length > 0 && dateColumn) {
    ...
    TimelineLogger.debug('[buildItemToMarkerMap] Map built via visibleBoardItems processor', { 
  ...
  TimelineLogger.debug('[buildItemToMarkerMap] Map built via fallback', { mapped: map.size });
