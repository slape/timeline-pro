...
export function generateDefaultPositions(items, position) {
  ...
  TimelineLogger.debug('Generating default positions for position setting change', {
    position,
  ...
  TimelineLogger.debug('Default positions generated', {
    ...
    samplePositions: Object.entries(defaultPositions).slice(0, 3).map(([id, pos]) => ({ id, y: pos.y }))
  });
 ...
 */
export function applyDefaultsForOutOfBoundsItems(customPositions, items, newPosition) {
