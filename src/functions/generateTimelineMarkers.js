...
const generateTimelineMarkers = (
  ...
  if (boardItems.length > 0 && dateColumnId) {
      ...
      if (column?.value) {
        try {
          ...
          const hasTimelineFields =
            columnValue.to || columnValue.end || columnValue.from;
          ...
          TimelineLogger.error("[Markers] Error parsing date value", {
            error,
            column: column?.value,
            itemId: item?.id,
          });
    ...
    TimelineLogger.debug("[Markers] Unique date strings found", {
      count: dates.size,
    ...
    if (uniqueDates.length > 0) {
      ...
      const actualStartDate = toDateOnly(
        new Date(Math.min(...uniqueDates.map((d) => d.getTime()))),
      );
      const actualEndDate = toDateOnly(
        new Date(Math.max(...uniqueDates.map((d) => d.getTime()))),
      );

      // Use the actual date range from board items, or fall back to provided start/end dates
      const timelineStart =
        actualStartDate < startDate ? actualStartDate : startDate;
      ...
      const uniqueMarkers = allMarkers.filter(
        (marker, index, self) =>
          index ===
          self.findIndex((m) => Math.abs(m.position - marker.position) < 0.1),
      );

      const sortedMarkers = uniqueMarkers.sort(
        (a, b) => a.position - b.position,
      );
      TimelineLogger.debug("[Markers] Generated from board items", {
    ...
    TimelineLogger.warn(
      "[Markers] No board items or date column, using fallback markers",
      { boardItemsCount: boardItems?.length || 0, hasDateColumn: !!dateColumn },
    );
